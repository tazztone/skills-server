 ...
 *   --allow-dirty            Allow dispatch from a non-clean git worktree. By default the relay
 *                            refuses ambiguous baselines before starting agy.
 ...
 * completed, completed_without_report, blocked_by_permission, failed, timeout
 * (the relay watchdog fired after --print-timeout plus 60s grace), aborted (the
 * relay itself was killed and forwarded the kill to agy), or agy_unavailable.
 * An orchestrator that polls for the
...
function permissionBlocked(stderrTail) {
  return stderrTail.some((line) =>
    /required the ["'](?:[^"']+)["'] permission|auto-denied|permission denied|permission request/i.test(line));
}
...
function makeResultWriter(opts, version, run) {
  return (extra) => {
    ...
    const result = {
      ...
      baselineTouchedFiles: run.baselineTouchedFiles ?? null,
      ...extra,
    };
...
function dispatchToAgy(opts, brief, run, writeResult) {
  ...
  child.on("close", (code, signal) => {
    ...
    const permissionError = permissionBlocked(stderrTail);
    const missingReport = succeeded && !finalMessage;
    const status = permissionError
      ? "blocked_by_permission"
      : succeeded
        ? missingReport ? "completed_without_report" : "completed"
        : watchdogFired ? "timeout" : "failed";
    const error = permissionError
      ? "agy exited without completing because a tool permission was denied; configure an allow-rule or explicitly approve --dangerously-skip-permissions before retrying"
      : missingReport
        ? "agy exited successfully but emitted no final report; inspect the working tree and logs before deciding whether to keep or re-dispatch"
        : watchdogFired
          ? `agy did not exit within --print-timeout ${opts.printTimeout} plus 60s grace; killed by the relay watchdog`
          : undefined;
    const result = writeResult({
      status,
      exitCode: status === "completed" ? 0 : mapped === 0 ? 1 : mapped,
      signal: signal ?? null,
      finalMessage,
      touchedFiles: gitTouchedFiles(opts.cd),
      ...((succeeded && !permissionError && !missingReport) ? {} : { stderrTail: stderrTail.slice(-20) }),
      ...(error ? { error } : {}),
    });
    printSummary(result, run.resultPath);
    process.exit(result.exitCode);
...
function main() {
  ...
  const baselineTouchedFiles = gitStatusSnapshot(opts.cd);
  if (baselineTouchedFiles === null) {
    fail(`--cd is not a readable git worktree: ${opts.cd}`);
  }
  if (baselineTouchedFiles.length > 0 && !opts.allowDirty) {
    fail(`git worktree is not clean (${baselineTouchedFiles.length} changed path${baselineTouchedFiles.length === 1 ? "" : "s"}); commit or stash existing changes, or pass --allow-dirty after recording the baseline`);
  }

  const version = agyVersion();
  const run = prepareRunDir(opts, brief);
  run.baselineTouchedFiles = baselineTouchedFiles;

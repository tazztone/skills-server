...
not sufficient. Only `status: completed` with a non-empty `finalMessage` is a complete delegation
handoff; inspect `completed_without_report` and `blocked_by_permission` as incomplete runs.
...
for. Compare it with `baselineTouchedFiles`; when `--allow-dirty` was used, separate pre-existing
changes from implementer changes before reviewing scope:
...
drop the now-unused import." | node "<skill-dir>/scripts/relay.mjs" --conversation <id> --cd /path/to/repo
```

Use the `conversationId` from the result when available; use `--resume-last` only when intentionally
continuing the most recent conversation. Then review again - rework gets the same gate-rerun, test
check, diff-read, and sweep as the original, no shortcuts.
